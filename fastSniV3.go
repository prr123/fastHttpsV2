...
//
// adapt fasthttpsMapV3 to Sni
// -- step 1: parse multi domain input
//
...
func main() {
	...
	// check domain
	if dbg {fmt.Printf("domain: %s\n", domStr)}
//	comma:= false
	var domList []string
	st :=0
	for i:= 1; i<len(domStr); i++ {
		if domStr[i] == ',' {
			domList = append(domList, string(domStr[st:i]))
			st =  i+1
		}
	}
	if st < len(domStr)-1 {
		domList = append(domList, string(domStr[st:]))
	}

	for i:=0; i<len(domList); i++ {
		domList[i] = strings.TrimSpace(domList[i])
	}
